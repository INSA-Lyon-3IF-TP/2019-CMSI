...
figure(1);
%F(609) = struct('cdata',[],'colormap',[]);
while max(abs(U - prevU)) > power(10,-2)
    prevU = U;
    U = cte * U;
    i = i + 1;
    surf(vectToMatrix(U))
    zlim([0 20]) % axe Z limité entre 0 et 20, pas de vue globale
    caxis([0 20]) % l'échelle des couleurs varie entre 0 et 20
    drawnow %affichage de la modélisation
    %F(i) = getframe;
end
%movie(F,0);
i % au bout d'environ 148 secondes (2 minutes 28s) l'équilibre est atteint 
